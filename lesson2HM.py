...
class GbBlogParse:
    ...
    def parse_post(self, response: requests.Response):
        ...
        date_tag = soup.find('time', attrs={'class': 'text-md text-muted m-r-md'})
        img = soup.find("div", attrs={"class": "blogpost-content"}).find("img")
        data = {
            'url': response.url,
            ...
            "first_img": img.get('src') if img else None,
            "datePublished": datetime.datetime.strptime(date_tag.get('datetime'), '%Y-%m-%dT%H:%M:%S%z'),
            'comments': self._get_comments(f'https://gb.ru/api/v2/comments?commentable_type=Post&'
                                           f'commentable_id={soup.find("comments")["commentable-id"]}&order=desc')
        }
        self._save(data)
